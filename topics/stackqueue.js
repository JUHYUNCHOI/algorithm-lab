...
int main() {
    ...
    printf("%lld\\n", sum);
        ...
        },
        {
            ...
            `,
            hints: [
                { title: '처음 떠오르는 방법', content: '여는 괄호랑 닫는 괄호 <strong>개수가 같으면</strong> 유효한 거 아닐까?<br><br><code>"()"</code> → 여는 1개, 닫는 1개 → 유효 ✓<br><code>"()[]{}"</code> → 여는 3개, 닫는 3개 → 유효 ✓<br><br>개수만 세면 간단하게 풀 수 있을 것 같은데…' },
                { title: '근데 이러면 문제가 있어', content: '<code>"(]"</code> → 여는 괄호 1개, 닫는 괄호 1개… <strong>개수는 맞는데 유효하지 않아요!</strong><br><code>"([)]"</code> → 여는 2개, 닫는 2개… 역시 개수는 맞지만 유효하지 않아요!<br><br>개수만으로는 <strong>"종류"</strong>(괄호 모양이 같은지)와 <strong>"순서"</strong>(안쪽 괄호를 먼저 닫는지)를 확인할 수 없습니다.<br>그럼 종류와 순서를 둘 다 체크하려면 어떻게 해야 할까요?' },
