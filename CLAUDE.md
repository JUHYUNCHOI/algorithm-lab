...
> **대상 수준**: USACO Bronze~Silver (또는 동등). 개념 데모와 문제 난이도는 이 범위를 기준으로 한다.
> Gold~Platinum 내용(매개변수 탐색, 플로이드-워셜, 세그먼트 트리 등)은 **심화 참고** 수준으로만 언급하고, 데모/문제의 핵심은 Silver 이하로 맞춘다.

| 카테고리 | 토픽 | 최소 데모 | 참조 |
|----------|------|-----------|------|
| 기초 (Bronze~Silver) | 문자열 | 3개 (인덱스/슬라이싱, ord/chr, 문자비교) | `string.js` |
| | 배열 | 3개 (인덱스 O(1), 삽입/삭제 비용, 투포인터) | `array.js` |
| | 정렬 | 7개 (선택/삽입/버블 + 비교표 + 속도비교 + 병합/퀵) | `sorting.js` |
| | 스택큐 | 5개 (Push/Pop, Enqueue/Dequeue, 양방향, 비교표, 괄호검증) | `stackqueue.js` |
| | 해시테이블 | 4개 (배열vs딕셔너리, 해시함수, 빈도수세기, 충돌/체이닝) | `hashtable.js` |
| 탐색 (Silver) | 이분탐색 | 3개 (이분탐색, 탐색실패, 크기비교) | `binarysearch.js` |
| | 재귀 | 5개 (마트료시카, 멈추는조건비교, 콜스택, 재귀vs반복, 3단계피보나치) | `recursion.js` |
| | 그래프 | 5개 (인접리스트만들기, DFS따라가기, BFS따라가기, 격자BFS, 유형맞추기퀴즈) | `graph.js` |
| | 누적합 | 4개 (누적합만들기, 쌓기과정, 반복문vs누적합, 유형맞추기) | `prefixsum.js` |
| 문제 해결 (Silver~Gold) | 백트래킹 | 6개 (결정트리, 3요소따라가기, 사이클데모, 가지치기비교, 실행비교, 4Queen) | `backtracking.js` |
| | 그리디 | 5개 (거스름돈, 실패경우비교, 활동선택타임라인, ATM줄서기, 3단계회의실) | `greedy.js` |
| | DP | 5개 (피보나치테이블, 재귀vsDP비교, 4단계계단오르기, TopDown-vs-BottomUp, 유형맞추기) | `dp.js` |
| | 분할정복 | 5개 (이진탐색, 합병정렬3단계, 겹침비교, 빠른거듭제곱, 색종이쿼드트리) | `divideconquer.js` |
| 심화 (Gold~Platinum) | 트리 | 4개 (용어확인, 노드추가, 전위순회, DFS깊이) | `tree.js` |
| | 트라이 | 3개 (트라이만들기, 검색따라가기, 자동완성) | `trie.js` |
| | 위상정렬 | 3개 (DAG진입차수, 순서맞추기, Kahn따라가기) | `topologicalsort.js` |
| | 우선순위큐 | 4개 (큐vs우선순위큐, 배열트리변환, SiftUp삽입, heapq시뮬레이터) | `priorityqueue.js` |
| | 유니온파인드 | 3개 (Union&Find체험, 경로압축, 사이클탐지) | `unionfind.js` |
| | 최단경로 | 3개 (BFS vs 다익스트라, 다익스트라스텝, 벨만포드완화) | `shortestpath.js` |
