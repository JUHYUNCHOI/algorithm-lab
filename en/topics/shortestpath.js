    ...
    problems: [
        ...
        // ===== Stage 2: Basic Shortest Path =====
        {
            ...
            hints: [
                ...
                { title: 'But there\'s a problem with this', content: 'In a weighted graph, using plain BFS means <strong>arriving first does not guarantee the shortest path!</strong><br>For example, if A to B costs 10 and A to C to B costs 2+3=5, the direct route (10) arrives at B first but the actual shortest is the detour (5).<br>So we need a method that "processes the closest vertex first"... and that is exactly <strong>Dijkstra algorithm</strong>!' },
        ...
        },
        {
            ...
            hints: [
                ...
                { title: 'What if we try this?', content: 'The <strong>Floyd-Warshall algorithm</strong> finds all-pairs shortest paths with a single triple nested loop!<br>Idea: "When going from i to j, would it be shorter <strong>via k</strong>?" Check for every k.<br><code>dp[i][j] = min(dp[i][j], dp[i][k] + dp[k][j])</code><br><br>Two important notes:<br>1. <strong>k (intermediate) must be the outermost loop</strong>! The k, i, j order is essential.<br>2. If there are <strong>multiple buses for the same route, store only the minimum</strong>!' },
