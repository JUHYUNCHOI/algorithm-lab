...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> 자동차 <strong>네비게이션</strong>을 생각해 보세요!<br><br>\
...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> "가장 가까운 곳부터 탐색하기"를 생각해 보세요!<br><br>\
...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> "모든 길을 반복해서 확인하기"를 생각해 보세요!<br><br>\
...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> "경유지를 하나씩 추가하기"를 생각해 보세요!<br><br>\
...
\
    ...
    // ====================================================================
    // Simulation 1: 다익스트라 기본 (boj-1753)
    // ====================================================================
    ...
    // ====================================================================
    // Simulation 2: 플로이드-워셜 (boj-11404)
    // ====================================================================
    ...
    // ====================================================================
    // Simulation 3: 최소비용 구하기 (boj-1916)
    // ====================================================================
    ...
    problems: [
        // ===== 1단계: 기본 최단 경로 =====
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '시작점에서 다른 모든 정점까지 최단 경로를 구해야 해요.<br>일단 떠오르는 건 <strong>BFS</strong>처럼 시작점에서 출발해서 이웃을 하나씩 방문하는 거예요.<br>근데 이 문제는 간선마다 <strong>가중치(비용)</strong>가 달라요. 가중치 없는 BFS는 "한 칸 = 1"이지만, 여기선 간선마다 비용이 다르니까 단순 BFS로는 안 돼요.' },
                { title: 'But there\'s a problem with this', content: '가중치가 있는 그래프에서 그냥 BFS를 쓰면, <strong>먼저 도착한 게 최단이 아닐 수 있어요!</strong><br>예를 들어 A→B 비용 10, A→C→B 비용 2+3=5이면, B에 먼저 도착하는 건 직행(10)이지만 실제 최단은 경유(5)에요.<br>그러면 "가장 가까운 정점부터 처리"하는 방법이 필요한데... 이게 바로 <strong>다익스트라 알고리즘</strong>이에요!' },
                { title: 'What if we try this?', content: '<strong>다익스트라</strong>의 핵심: 아직 확정 안 된 정점 중 <strong>거리가 가장 짧은 것</strong>부터 꺼내서 처리해요.<br>① dist 배열을 INF로 초기화하고, 시작점만 0으로 설정<br>② <strong>최소 힙</strong>에 (0, 시작점)을 넣어요<br>③ 힙에서 꺼낸 (거리, 정점)이 이미 확정된 거리보다 크면 → 무시!<br>④ 이웃 정점의 거리를 갱신할 수 있으면 갱신하고 힙에 추가<br>이렇게 하면 O((V+E) log V)로 모든 정점까지의 최단 거리를 구할 수 있어요!' },
                { title: 'In Python/C++!', content: '<span class="lang-py">Python에선 <code>heapq</code> 모듈로 최소 힙을 쓸 수 있어요.<br><code>heapq.heappush(heap, (거리, 정점))</code>으로 넣고, <code>heapq.heappop(heap)</code>으로 가장 가까운 걸 꺼내요.<br>힙이 알아서 거리순 정렬을 유지해주니까, 우리는 그냥 넣고 빼기만 하면 돼요!</span><span class="lang-cpp">C++에선 <code>priority_queue</code>에 <code>greater&lt;pair&lt;int,int&gt;&gt;</code>를 넣어 최소 힙을 만들어요.<br><code>pq.push({거리, 정점})</code>으로 넣고, <code>pq.top()</code> + <code>pq.pop()</code>으로 가장 가까운 걸 꺼내요.<br>C++ priority_queue는 기본이 최대 힙이라 <code>greater</code>를 꼭 써야 최소 힙이 돼요!</span>' }
            ],
            templates: {
        ...
        // ===== 2단계: 최단 경로 응용 =====
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: 'A 도시에서 B 도시까지 가는 <strong>최소 비용</strong>을 구해야 해요.<br>일단 가장 단순하게, A에서 B까지 가능한 <strong>모든 경로</strong>를 탐색해서 비용을 비교하면 어떨까요?<br>DFS로 A에서 출발해서 B에 도착하는 모든 경로의 비용을 구하고, 그 중 최솟값을 찾는 거예요.' },
                { title: 'But there\'s a problem with this', content: '모든 경로를 탐색하면 경로 수가 <strong>지수적으로</strong> 늘어나요!<br>도시가 1,000개이고 버스가 100,000개면, 가능한 경로가 어마어마하게 많아서 시간 초과가 나요.<br>이전 문제(1753번)에서 배운 <strong>다익스트라</strong>를 쓰면 훨씬 빠르게 해결할 수 있어요!' },
                { title: 'What if we try this?', content: '1753번과 거의 같은 구조예요! <strong>다익스트라</strong>로 출발 도시 A에서 모든 도시까지의 최단 거리를 구해요.<br>다른 점은 딱 하나: 마지막에 <strong>모든 정점의 거리</strong>를 출력하는 대신, <strong>도착 도시 B의 거리만</strong> 출력하면 끝!<br><br>⚠️ 주의: 같은 출발-도착에 <strong>여러 버스</strong>가 있을 수 있어요.<br>하지만 인접 리스트에 모두 추가하면 다익스트라가 알아서 최솟값을 찾아줘요!' },
        ...
        },
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '노드 k에서 신호를 보내면 <strong>모든 노드가 신호를 받는 데 걸리는 시간</strong>을 구해야 해요.<br>일단 가장 직관적인 방법: k에서 모든 노드까지 가능한 경로를 전부 탐색(DFS/BFS)해서, 각 노드에 도달하는 최소 시간을 구하는 거예요.<br>그리고 그 중 가장 큰 값이 "모든 노드가 신호를 받는 시간"이에요.' },
                { title: 'But there\'s a problem with this', content: '모든 경로를 탐색하면 중복 방문이 많아져서 느려요!<br>간선이 최대 6,000개이고 가중치가 있으니까, 단순 BFS로도 최단 시간을 보장할 수 없어요.<br>가중치가 있는 그래프에서 한 점 → 모든 점 최단 경로... 이건 <strong>다익스트라</strong>가 딱이에요!' },
                { title: 'What if we try this?', content: '<strong>다익스트라</strong>로 k에서 모든 노드까지의 최단 시간을 구해요. 여기까진 1753번과 같아요!<br><br>그런데 이 문제는 한 가지가 더 있어요: <strong>"모든 노드가 신호를 받는 시간"</strong>이 정답이에요.<br>신호는 동시에 퍼져나가니까, 가장 <strong>늦게 도착하는 노드의 시간 = 전체 시간</strong>이에요.<br>→ dist 배열에서 <strong>최댓값</strong>을 구하면 끝!<br><br>⚠️ 한 가지 더: 도달 불가능한 노드가 있으면(dist가 INF) <strong>-1</strong>을 반환해야 해요.' },
                { title: 'In Python/C++!', content: '<span class="lang-py">다익스트라 후 <code>max(dist[1:n+1])</code>로 최댓값을 구해요.<br>이 값이 <code>float(\'inf\')</code>이면 도달 불가능한 노드가 있다는 뜻이니까 -1을 반환해요.<br><code>return ans if ans != INF else -1</code> 한 줄로 깔끔하게 처리!</span><span class="lang-cpp">다익스트라 후 <code>*max_element(dist.begin()+1, dist.end())</code>로 최댓값을 구해요.<br>이 값이 INF(1e9)이면 도달 불가능한 노드가 있으므로 -1을 반환해요.<br><code>return ans == INF ? -1 : ans;</code> 삼항 연산자로 간결하게 처리!</span>' }
            ],
            templates: {
            ...
            solutions: [{
                ...
                codeSteps: {
                    ...
                    cpp: [
                        ...
                        { title: 'Return Result', desc: 'max_element로 dist[1]~dist[n] 중 최대값 확인.', code: 'int ans = *max_element(dist.begin()+1, dist.end());\nreturn ans == INF ? -1 : ans;' }
                    ]
                },
