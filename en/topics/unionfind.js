        ...
        container.innerHTML = `
            ...
            <div class="concept-section">
                ...
                <div class="analogy-box">
                    <strong>Understanding by analogy:</strong> "동아리 합치기"를 생각해 보세요!<br><br>
...
int find(int x) {
    if (parent[x] != x)
        parent[x] = find(parent[x]);  // Path 압축!
    return parent[x];
}
    ...
    if (a != b)
        parent[b] = a;  // b의 대표를 a로 변경
                ...
                <div style="margin:0.8rem 0 0.5rem;">
                    <span class="lang-cpp"><a href="https://en.cppreference.com/w/cpp/algorithm/iota" target="_blank" style="font-size:0.85rem;color:var(--accent);text-decoration:underline;">C++ Reference: iota (순차 값 채우기) ↗</a></span>
            ...
            <div class="concept-section">
                ...
                <div class="analogy-box">
                    <strong>Understanding by analogy:</strong> 아무 생각 없이 합치면 트리가 일자로 길어질 수 있습니다.<br>
...
sort(edges.begin(), edges.end(), [](auto&amp; a, auto&amp; b) {
    return get&lt;2&gt;(a) &lt; get&lt;2&gt;(b);  // Weight 기준 정렬
});
...
cout &lt;&lt; mst_cost &lt;&lt; endl;</code></pre>
    ...
    // ====================================================================
    // Simulation 1: 기본 유니온 파인드 (boj-1717)
    // ====================================================================
    ...
    problems: [
        // ===== 1단계: 기본 유니온 파인드 =====
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '집합을 관리하려면... 각 집합을 <span class="lang-py"><strong>리스트(list)</strong></span><span class="lang-cpp"><strong>벡터(vector)</strong></span>로 만들면 되지 않을까?<br><br>"0 a b"가 오면 a가 속한 리스트와 b가 속한 리스트를 합치고,<br>"1 a b"가 오면 a와 b가 같은 리스트에 있는지 확인하면 될 것 같아!' },
                { title: 'But there\'s a problem with this', content: '합집합할 때 한쪽 리스트의 원소를 전부 다른 쪽으로 옮겨야 해. 원소가 k개면 <strong>O(k)</strong>번 이동이 필요하고...<br><br>n이 최대 <strong>1,000,000</strong>, m이 최대 <strong>100,000</strong>이면, 최악의 경우 합칠 때마다 수십만 개를 옮겨야 할 수도 있어. 너무 느려! 😱<br><br>같은 집합인지 확인하는 것도, 리스트를 다 뒤져야 하면 <strong>O(n)</strong>이 걸려.' },
                { title: 'What if we try this?', content: '리스트 대신 <strong>트리 구조</strong>를 쓰자! 각 원소가 "부모"를 가리키게 하면:<br><br>• <strong>find(x)</strong>: x에서 부모를 따라 올라가면 루트를 찾을 수 있어. 루트가 같으면 같은 집합!<br>• <strong>union(a, b)</strong>: 한쪽 루트를 다른 쪽 루트의 자식으로 붙이면 끝!<br><br>여기에 두 가지 최적화를 더하면 거의 <strong>O(1)</strong>에 가까워져:<br>① <strong>경로 압축</strong> — find할 때 만나는 노드를 전부 루트에 직접 연결<br>② <strong>랭크 기반 합치기</strong> — 높이가 낮은 트리를 높은 쪽에 붙임' },
            ...
            solutions: [{
                ...
                codeSteps: {
                    python: [
                        ...
                        { title: 'Initialize', desc: '각 노드가 자기 자신을 부모로 가리키도록 초기화합니다. 처음엔 모두 독립 집합입니다.', code: 'n, m = map(int, input().split())\nparent = list(range(n + 1))\nrank = [0] * (n + 1)' },
                    ...
                    cpp: [
                        ...
                        { title: 'Initialize', desc: '전역 배열이므로 main에서 0~n까지 자기 자신으로 초기화합니다.', code: 'int main() {\n    int n, m;\n    scanf("%d %d", &n, &m);\n    for (int i = 0; i <= n; i++) {\n        parent[i] = i;\n        rnk[i] = 0;\n    }' },
        ...
        },
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '여행 경로가 1→2→3이면... 1에서 2로 갈 수 있는지, 2에서 3으로 갈 수 있는지 하나씩 확인하면 되지 않을까?<br><br>인접 행렬에서 직접 연결을 확인하거나, 직접 연결이 없으면 <strong>BFS/DFS</strong>로 경로가 있는지 탐색해볼 수 있겠어!' },
                { title: 'But there\'s a problem with this', content: '잠깐, 여행 경로가 M개 도시를 거치면 매번 BFS/DFS를 돌려야 해. 최악의 경우 <strong>O(M * N^2)</strong>이 될 수 있어.<br><br>그런데 사실... 꼭 "1→2→3 순서대로" 갈 수 있는지를 볼 필요가 있을까? 🤔<br>문제를 다시 읽어보면, 같은 도시를 여러 번 방문해도 되고, 경로가 어떻든 상관없어. 그러면 핵심은 뭘까?' },
                { title: 'What if we try this?', content: '핵심 관찰: 여행 도시들이 전부 <strong>같은 연결 요소</strong>에 있기만 하면 여행이 가능해!<br><br>연결만 되어 있으면 어떤 경로로든 오갈 수 있으니까. 그러면 문제가 단순해져:<br><br>① 인접 행렬에서 연결된 도시 쌍을 전부 <strong>union</strong><br>② 여행 도시들의 <strong>find</strong> 값이 모두 같으면 YES!<br><br>N이 최대 200이라 인접행렬을 읽으며 union하면 O(N^2)이면 충분해. 유니온 파인드로 깔끔하게 해결!' }
            ],
            templates: {
  ...
  ["0","0","0","1","1"]
            ...
            `,
            hints: [
                { title: 'First intuition', content: '격자에서 섬을 찾으려면... \'1\'인 칸을 만나면 거기서 <strong>BFS나 DFS</strong>로 연결된 \'1\'을 전부 방문 처리하면 되겠지?<br><br>탐색을 시작한 횟수 = 섬 개수! 이건 그래프 탐색의 기본 패턴이야.' },
                ...
                { title: 'What if we try this?', content: '구체적인 방법:<br><br>① 2D 좌표 (r, c)를 <code>r * cols + c</code>로 <strong>1D 인덱스</strong>로 변환 (parent 배열에 쓰려고!)<br>② 처음 섬 개수 = \'1\'인 칸의 총 개수<br>③ 모든 \'1\' 칸을 순회하면서, <strong>오른쪽/아래</strong> 칸도 \'1\'이면 union<br>④ union이 성공할 때마다 (서로 다른 집합이었을 때) 섬 개수를 <strong>1 감소</strong><br><br>왜 오른쪽/아래만? 왼쪽/위는 이미 이전에 확인했으니까 중복을 피하는 거야!' },
                { title: 'In Python/C++!', content: '<span class="lang-py">Python에서는 <code>parent = list(range(rows * cols))</code>로 간단히 초기화하고, union 성공 여부를 <code>return True/False</code>로 반환하면 count 감소를 깔끔하게 처리할 수 있어!</span><span class="lang-cpp">C++에서는 <code>iota(parent.begin(), parent.end(), 0)</code>으로 0,1,2,...를 한 번에 초기화할 수 있어! unite가 <code>bool</code>을 반환하게 만들면 성공 시 count--를 깔끔하게 처리할 수 있지.</span>' }
            ],
            templates: {
