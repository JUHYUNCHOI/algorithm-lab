...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> 응급실에서는 먼저 온 사람이 아니라, <strong>가장 위급한 환자</strong>부터 치료합니다.<br><br>\
...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> 힙은 <strong>완전 이진 트리</strong>라는 특별한 나무 모양입니다.<br>\
...
\
            ...
            ops.forEach(function(op) {
                if (op.type === 'push') {
                    ...
                    if (heap.length === 0) {
                        ...
                        states.push({ h: heap.slice(), msg: '최댓값 ' + popped + '을(를) 꺼냄! 힙: [' + (heap.length ? heap.join(', ') : 'Empty') + ']' });
                    }
                }
            });
            var steps = [];
            ...
            ops.forEach(function(op) {
                if (op.type === 'push') {
                    ...
                    if (heap.length === 0) {
                        ...
                        var popped = heap.shift();
                        states.push({ h: heap.slice(), msg: '최솟값 ' + popped + ' 꺼냄! 힙: [' + (heap.length ? heap.join(', ') : 'Empty') + ']' });
                    }
                }
            });
            var steps = [];
            ...
            seq.forEach(function(x) {
                if (x !== 0) {
                    ...
                    if (heap.length === 0) {
                        ...
                        var popped = heap.shift();
                        states.push({ h: heap.slice(), msg: '절댓값 최소 ' + popped + ' 꺼냄! 힙: [' + (heap.length ? heap.join(', ') : 'Empty') + ']' });
                    }
                }
            });
            var steps = [];
    ...
    _renderVizJewelThief: function(container) {
        ...
        function renderH(h, msg) {
            heapEl.innerHTML = h.length === 0 ? '<span style="color:var(--text3);">힙 Empty</span>' :
...
0</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '"가장 큰 값을 출력하라"니까... 배열에 숫자를 넣고, 0이 나올 때마다 <strong>배열 전체를 뒤져서 최댓값</strong>을 찾으면 되지 않을까?<br><br>매번 <code>max()</code>로 찾고, 그 값을 제거하면 될 것 같아!' },
                { title: 'But there\'s a problem with this', content: '배열에서 최댓값 찾기 = <strong>O(N)</strong>, 제거도 <strong>O(N)</strong>이야.<br>연산이 최대 10만 번이고, 매번 O(N)이면 <strong>10만 × 10만 = 100억 번</strong>... 시간 초과!<br><br>넣을 때는 빠르지만, <strong>꺼낼 때마다 전부 뒤지는 게 병목</strong>이야.' },
                { title: 'What if we try this?', content: '<strong>힙(Heap)</strong>을 쓰면 삽입도 O(log N), 최댓값 꺼내기도 O(log N)이야!<br>힙은 "항상 최댓값(또는 최솟값)이 맨 위에 있는" 특별한 트리 구조거든.<br><br>그런데 주의할 점이 하나 있어:<br><span class="lang-py">Python의 <code>heapq</code>는 <strong>최소 힙</strong>만 지원해. 최대 힙이 필요한데 어떡하지? 🤔<br>→ <strong>-1을 곱해서</strong> 넣으면 돼! 가장 작은 음수 = 원래 가장 큰 수니까!</span><span class="lang-cpp">C++의 <code>priority_queue&lt;int&gt;</code>는 <strong>기본이 최대 힙</strong>이라 그대로 쓰면 돼! Python보다 오히려 간단하지.</span>' },
...
0</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '"가장 작은 값을 출력하라"니까... 숫자를 배열에 넣고, 0이 나오면 <strong>배열을 정렬해서 맨 앞</strong>을 꺼내면 되지 않을까?<br><br>아니면 매번 <code>min()</code>으로 최솟값을 찾아서 제거하거나!' },
                { title: 'But there\'s a problem with this', content: '매번 정렬하면 <strong>O(N log N)</strong>, min()으로 찾아도 <strong>O(N)</strong>이야.<br>연산이 최대 10만 번이면 <strong>최악 10만 × 10만 = 100억 번</strong>... 너무 느려!<br><br>최대 힙 문제랑 똑같은 상황이야. <strong>"매번 전체를 뒤지는"</strong> 게 문제지.' },
                { title: 'What if we try this?', content: '<strong>최소 힙</strong>을 쓰면 삽입 O(log N), 최솟값 꺼내기 O(log N)으로 해결돼!<br>힙은 "맨 위에 항상 최솟값(또는 최댓값)이 있는" 구조라서, 매번 전체를 안 뒤져도 바로 꺼낼 수 있어.<br><br>최대 힙 문제에선 -1 곱하기 트릭이 필요했는데, <strong>최소 힙은 그냥 넣으면 돼!</strong>' },
...
0</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '"절댓값이 가장 작은 값을 꺼내라"니까... 배열에 숫자를 넣고, 0이 나오면 <strong>전부 절댓값을 비교해서</strong> 가장 작은 걸 찾으면 되지 않을까?<br><br>절댓값이 같은 게 여러 개면 그중 실제 값이 작은 걸 고르면 되고!' },
                { title: 'But there\'s a problem with this', content: '매번 전체를 훑으면 <strong>O(N)</strong>이야. 연산이 최대 10만 번이면 또 시간 초과!<br><br>힙을 쓰면 될 것 같은데... 문제는 <strong>"절댓값 기준"</strong>이라는 거야.<br>일반 최소 힙은 실제 값 기준으로 정렬하니까, -1과 1 중에 -1을 먼저 꺼내버려. 우리는 둘 다 절댓값이 1이니까 동점 처리가 필요한데!' },
                { title: 'What if we try this?', content: '힙에 숫자를 그냥 넣지 말고, <strong>(절댓값, 실제 값)</strong> 쌍으로 넣으면 어떨까?<br><br>예: 1 → (1, 1), -1 → (1, -1), 3 → (3, 3)<br><br>힙이 첫 번째 값(절댓값)으로 먼저 정렬하고, 같으면 두 번째 값(실제 값)으로 정렬하니까, <strong>절댓값이 같을 때 음수가 먼저</strong> 나오게 돼! 정확히 문제가 원하는 동작이야.' },
...
14 24 32 39 48</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: 'N번째 큰 수를 찾으라니까... N\u00B2개 숫자를 <strong>전부 배열에 넣고 정렬</strong>해서 N번째를 꺼내면 되지 않을까?<br><br>N이 1,500이면 숫자가 1,500 \u00D7 1,500 = <strong>225만 개</strong>. 정렬하면 답은 나올 것 같아!' },
                { title: 'But there\'s a problem with this', content: '문제를 다시 보면... <strong>메모리 제한이 12MB</strong>야!<br>int 225만 개 = 약 9MB인데, 배열 자체만으로도 빠듯하고 정렬에 추가 메모리까지 필요하면 초과할 수 있어.<br><br>핵심은 "225만 개를 <strong>전부 저장하지 않고</strong>도 N번째 큰 수를 찾을 수 있느냐"야!' },
                { title: 'What if we try this?', content: '생각해보면, <strong>"상위 N개"만 기억</strong>하면 되잖아!<br><br><strong>크기 N짜리 최소 힙</strong>을 유지하는 거야:<br>1. 힙 크기가 N 미만이면 그냥 넣기<br>2. 힙 크기가 N이면, 새 값이 힙의 최솟값(루트)보다 <strong>클 때만</strong> 교체<br><br>이러면 힙에는 항상 "지금까지 본 숫자 중 가장 큰 N개"만 남아. 다 읽은 후 <strong>힙의 루트(최솟값) = N번째 큰 수</strong>!<br>메모리도 N개만 저장하니까 12MB 여유있게 통과해.' },
...
23 23 22 22 13 3 5 5 3 0 0 -2</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '홀수 번째 수를 읽을 때마다 "지금까지의 중앙값"을 출력하라니까...<br>매번 <strong>지금까지 읽은 숫자를 정렬</strong>하고 가운데 값을 꺼내면 되지 않을까?<br><br>예: [1, 2, 3, 4, 5] → 정렬 → 가운데(3번째) = 3!' },
                { title: 'But there\'s a problem with this', content: '숫자가 하나 들어올 때마다 전체를 정렬하면 <strong>O(N log N)</strong>이야.<br>M이 최대 9,999이면 매 홀수 번째마다 정렬 → 약 5,000 \u00D7 10,000 \u00D7 log(10,000) \u2248 <strong>6억 번</strong>... 시간 초과 위험!<br><br>숫자가 하나 추가될 때마다 <strong>정렬을 다시 하는 건 낭비</strong>야. 이미 정렬된 상태에서 하나만 추가되는 건데...' },
                { title: 'What if we try this?', content: '핵심 아이디어: 수열을 <strong>절반으로 나눠서</strong> 관리하자!<br><br>\u2022 <strong>최대 힙(maxH)</strong>: 작은 쪽 절반 → 이 중 가장 큰 값이 루트<br>\u2022 <strong>최소 힙(minH)</strong>: 큰 쪽 절반 → 이 중 가장 작은 값이 루트<br><br>두 힙의 크기를 균형 맞추면 (maxH 크기 \u2265 minH 크기), <strong>maxH의 루트가 바로 중앙값</strong>이야!<br><br>새 숫자가 들어오면:<br>1. maxH 루트보다 작으면 maxH에, 크면 minH에 넣기<br>2. 크기가 불균형하면 한쪽에서 다른 쪽으로 옮기기<br><br>삽입+균형 맞추기가 <strong>O(log N)</strong>이라서 전체 <strong>O(M log M)</strong>으로 해결!' },
...
2</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '가방마다 보석 하나씩 넣으니까... 각 가방에 <strong>들어갈 수 있는 보석 중 가장 비싼 걸</strong> 넣으면 최대 이득 아닌가?<br><br>가방마다 모든 보석을 확인해서 "무게가 맞고 가장 비싼 것"을 골라 넣자!' },
                { title: 'But there\'s a problem with this', content: 'N, K가 최대 30만이면, 가방 30만 개 \u00D7 보석 30만 개 = <strong>900억 번</strong> 비교... 완전히 시간 초과!<br><br>게다가 한 보석을 가방 A에 넣으면 가방 B에 못 넣으니까, <strong>어떤 가방에 어떤 보석을 배정하느냐</strong>가 중요해. 단순 완전 탐색으론 안 돼.' },
                { title: 'What if we try this?', content: '<strong>그리디 전략</strong>: 가방을 <strong>용량이 작은 순서</strong>대로 처리하자!<br><br>왜 작은 가방부터? 작은 가방에 들어가는 보석은 큰 가방에도 들어가지만, 반대는 아니거든. 그래서 <strong>선택지가 적은 가방부터</strong> 처리하는 게 최적이야.<br><br>각 가방을 처리할 때:<br>1. 이 가방에 <strong>무게가 맞는 보석</strong>을 전부 최대 힙에 넣기<br>2. 힙에서 <strong>가장 비싼 보석</strong>을 하나 꺼내기<br><br>보석도 무게순 정렬해두면, 포인터 하나로 "아직 안 넣은 보석 중 무게가 맞는 것"을 순서대로 넣을 수 있어!' },
