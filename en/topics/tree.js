    ...
    problems: [
        ...
        },
        {
            ...
            hints: [
                ...
                { title: 'Simple with recursion', content: 'Recursion makes this very clean:<br><br>1. <strong>Swap</strong> the current node\'s <code>left</code> and <code>right</code><br>2. Recursively invert the left subtree<br>3. Recursively invert the right subtree<br><br>Base case: if the node is empty (<span class="lang-py"><code>None</code></span><span class="lang-cpp"><code>nullptr</code></span>), just return.' },
        ...
        // ===== Stage 2: Tree Applications =====
        {
            ...
            hints: [
                ...
                { title: 'BFS is perfect!', content: '<strong>BFS (Breadth-First Search)</strong> inherently explores level by level, making it a perfect fit for this problem!<br><br>Key trick: <strong>pop only as many nodes as the current queue size</strong>, and exactly one level is processed.<br><br>As we dequeue nodes and enqueue their children, the next level naturally builds up in the queue.' },
...
DBEGFCA</pre></div>
            ...
            `,
            hints: [
                { title: 'Pre/In/Postorder — only the order differs', content: 'All three traversals visit in the <strong>left -> right</strong> direction, but differ in <strong>"when to print the current node"</strong>:<br><br><strong>Preorder</strong>: print first -> left -> right (root comes first)<br><strong>Inorder</strong>: left -> print -> right (root in the middle)<br><strong>Postorder</strong>: left -> right -> print (root comes last)<br><br>Remember this and the recursive code writes itself!' },
