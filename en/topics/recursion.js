...
const recursionTopic = {
    ...
    _createStepControls(suffix) {
        return '<div class="viz-step-controls">' +
            '<button class="btn viz-step-btn" id="str-prev-' + suffix + '" disabled>◀ Prev</button>' +
            '<span id="str-indicator-' + suffix + '" class="viz-step-counter">Before Start</span>' +
            '<button class="btn btn-primary viz-step-btn" id="str-next-' + suffix + '">Next ▶</button>' +
            '</div>' +
            '<div id="str-desc-' + suffix + '" class="viz-step-desc">▶ Click Next to start</div>';
    },
    ...
    _renderVizFibonacci(el) {
        ...
        const initViz = (n) => {
                ...
                <div class="viz-panels-grid">
                    ...
                    <div class="viz-panel">
                        <div class="viz-panel-header">
                            <h3>Call Count</h3>
                            <div class="counter">Total: <span id="call-count" class="counter-num">0</span> calls</div>
                        </div>
                        <div class="viz-panel-body">
            ...
            steps.push({
                description: `✅ fib(${n}) = ${fib[n]}, total ${totalCalls || rawSteps.filter(s=>s.type==='call').length} calls! Lots of duplicates, right? → This is solved with DP`,
                action() {},
                undo() {}
            });
    ...
    _renderVizFibSim(container) {
        ...
        function buildAndRender(n) {
            ...
            steps.push({ description: '✅ fib(' + n + ') = ' + fibResult + ', total ' + totalCalls + ' calls!', action: function(){}, undo: function(){} });
    ...
    _renderVizMergeSim(container) {
        ...
        function buildAndRender(arr, k) {
            container.innerHTML =
                ...
                '</div>' +
                '<div class="viz-panel"><div class="viz-panel-header"><h3>Merge Sort: [' + arr.join(', ') + ']</h3></div>' +
                '<div class="viz-panel-body">' +
            ...
            function recordMergeSort(a, p, r, segments) {
                ...
                snapshots.push({
                    desc: 'Merge: [' + p + '..' + q + '] + [' + (q+1) + '..' + r + '] → [' + tmp.join(', ') + '] (save #' + startSave + '~' + endSave + ')',
                    array: a.slice(),
                    mergedStart: p,
                    mergedEnd: r,
                    saveStart: startSave,
            ...
            snapshots.forEach(function(snap) {
                ...
                if (k >= snap.saveStart && k <= snap.saveEnd) {
                    kInfo = '<div style="margin-top:8px;color:var(--accent);">K=' + k + ' saved value: <strong>' + kthValue + '</strong></div>';
                }
                stepsData.push({
            ...
            var finalDesc = kthValue !== -1
                ? '✅ Done! K=' + k + ' saved value: ' + kthValue
                : '✅ Done! Total saves fewer than K=' + k + ', result is -1';
            stepsData.push({
    ...
    problems: [
        // ========== Stage 1: Recursion Basics ==========
        {
            ...
            hints: [
                ...
                { title: 'But how with recursion?', content: 'Look closely: <code>5! = 5 × 4!</code>, and <code>4! = 4 × 3!</code>. In other words, <code>n! = n × (n-1)!</code> — the big problem breaks down into a smaller problem! This is the core of recursion.' },
...
4 5 1 3 2</pre></div>
            ...
            hints: [
                ...
                { title: 'Where are values stored?', content: 'In the <strong>merge step</strong>! Merge sort has two phases: "splitting (divide)" and "combining (merge)." Values are actually copied into array A only during the merge step. Each time a value is moved from the temporary array back to A counts as one store.' },
        ...
        // ========== Stage 3: Divide and Solve ==========
        {
            ...
            hints: [
                ...
                { title: 'This is a recursive structure!', content: 'Divide a segment of length len into three parts: 1) recurse on the left 1/3, 2) replace the middle 1/3 with spaces, 3) recurse on the right 1/3. "Remove the middle from the whole, then repeat on both remaining sides" — classic divide and conquer!' },
...
1 3</pre></div>
            ...
            hints: [
                ...
                { title: 'Key idea', content: 'Break the problem of moving N disks from peg 1 to peg 3 into <strong>3 steps</strong>! 1) Move the top N-1 disks to peg 2 (so the largest disk is exposed), 2) Move the largest disk to peg 3, 3) Move the N-1 disks from peg 2 to peg 3. "Moving the top N-1 disks" is the same problem — that is recursion!' },
