...
const recursionTopic = {
        ...
        container.innerHTML = `
            ...
            <div class="concept-section">
                ...
                <div class="analogy-box matryoshka-section">
                    <strong>Understanding by analogy:</strong> 러시아 인형(마트료시카)을 생각해보세요.
    ...
    problems: [
        // ========== 1단계: 재귀 입문 ==========
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '<code>5! = 5 × 4 × 3 × 2 × 1</code>이니까 for문으로 곱하면 되지 않을까? 맞아, 그것도 방법이야! 근데 이 문제는 <strong>재귀</strong>를 연습하는 문제야.' },
        ...
        },
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '피보나치 수열은 <code>0, 1, 1, 2, 3, 5, 8, 13, ...</code> — 앞 두 수를 더하면 다음 수가 나와! for문으로 앞에서부터 하나씩 구하면 될 것 같은데?' },
...
0 3</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '문제에서 재귀 함수 <code>recursion(s, l, r)</code>이 이미 주어졌어! 양쪽 끝 글자를 비교하고, 같으면 안쪽으로 좁혀가는 구조야. 일단 그대로 구현하면 회문 판별은 되는데... 호출 횟수는 어떻게 세지?' },
                ...
                { title: 'What if we try this?', content: '매 테스트케이스마다 카운터를 0으로 <strong>초기화</strong>하는 걸 잊지 마! 안 그러면 이전 문자열의 횟수가 누적돼. <code>isPalindrome</code> 안에서 <code>cnt = 0</code>으로 리셋하고 <code>recursion</code>을 호출하면 깔끔해.' },
...
4 5 1 3 2</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '병합 정렬을 그냥 구현하면 정렬은 되는데... 문제는 "K번째로 저장되는 수"를 찾는 거야. 정렬 과정에서 배열에 값이 <strong>써지는 순서</strong>를 추적해야 해. 어디서 값이 써지지?' },
                ...
                { title: 'What if we try this?', content: '전역 카운터 <code>cnt</code>를 만들어서, merge에서 A에 값을 쓸 때마다 <code>cnt += 1</code>. cnt가 K가 되는 순간의 값을 <code>result</code>에 저장하면 끝! merge_sort 자체의 재귀 구조(<code>q = (p+r)//2</code>로 반 나누고, 왼쪽/오른쪽 각각 정렬 후 merge)는 교과서 그대로 구현하면 돼.' },
        ...
        // ========== 3단계: 나누어 풀기 ==========
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: 'N=2면 길이 9짜리 <code>---------</code>에서 가운데 3개를 공백으로 바꾸면... <code>---   ---</code>? 근데 양쪽 <code>---</code> 안에서도 또 가운데를 빼야 해! 한 번만 하는 게 아니라 <strong>반복적으로 쪼개야</strong> 하는 구조네.' },
        ...
        },
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '크기 3이면 가운데만 비운 3×3 패턴이야. 크기 9면? 9×9를 3×3 블록 9개로 나눠서 가운데 블록을 통째로 비우고, 나머지 8개 블록 안에서도 같은 패턴을 반복... 이거 칸토어 집합이랑 비슷한데, 1차원이 아니라 <strong>2차원</strong>이네!' },
...
1 3</pre></div>
            ...
            `,
            hints: [
                { title: 'First intuition', content: '원판 1개는 쉬워 — 그냥 옮기면 돼. 원판 2개도 해볼 만해: 작은 걸 보조 기둥에, 큰 걸 목표에, 작은 걸 다시 목표에. 근데 원판 3개, 4개는... 머리가 복잡해지는데, 일일이 시뮬레이션하지 않고 <strong>패턴</strong>을 찾을 수 없을까?' },
                ...
                { title: 'What if we try this?', content: '<code>hanoi(n, from, to, via)</code> 함수를 만들어: n개를 from→to로, 보조 기둥은 via. 멈추는 조건은 <code>n = 0</code>이면 옮길 게 없으니 return. <code>n = 1</code>이면 바로 from→to 출력해도 되지만, n=0으로 멈춰도 자연스럽게 동작해!' },
