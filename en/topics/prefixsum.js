...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> 여러분이 매일 용돈을 저금통에 넣는다고 생각해 보세요.<br>\
...
\
            <div class="concept-section">\
                ...
                <div class="analogy-box">\
                    <strong>Understanding by analogy:</strong> 격자 모양 지도에서 각 칸에 보물이 있다고 생각해 보세요.<br>\
...
\
    ...
    // ====================================================================
    // Simulation 1: 1D 구간 합 (boj-11659)
    // ====================================================================
    ...
    // ====================================================================
    // Simulation 3: 문자별 누적합 (boj-16139)
    // ====================================================================
    ...
    problems: [
        // ========== 1단계: 1차원 입문 ==========
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '구간 합을 구하라고 하니까, 매 질문마다 i부터 j까지 반복문을 돌려서 더하면 되지 않을까?<br><br><strong>예: [5, 4, 3, 2, 1]에서 1~3 구간 합</strong><br>→ arr[1] + arr[2] + arr[3] = 5 + 4 + 3 = 12<br>한 번의 질문에 O(N)이면 충분해 보여!' },
                { title: 'But there\'s a problem with this', content: '질문이 <strong>최대 100,000번</strong>이고 수도 100,000개야.<br>매번 반복문으로 더하면 최악의 경우 100,000 × 100,000 = <strong>100억 번 연산</strong>... 시간 초과 확정!<br><br>질문 하나에 O(N)이면 전체 O(N×M). 이걸 줄여야 해.' },
                { title: 'What if we try this?', content: '미리 <strong>누적합 배열</strong>을 만들어두면 어떨까?<br><code>prefix[k]</code> = 1번째부터 k번째까지의 합<br><br>그러면 i~j 구간 합 = <code>prefix[j] - prefix[i-1]</code> 한 번의 뺄셈으로 끝!<br>전처리 O(N) + 각 질문 O(1) = 전체 <strong>O(N + M)</strong> 🎯<br><br>💡 팁: <code>prefix[0] = 0</code>으로 두면 i=1일 때도 <code>prefix[1-1] = prefix[0] = 0</code>이라 예외 처리가 필요 없어!' }
            ],
            templates: {
        ...
        },
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '"연속 K일의 온도 합이 최대"니까, 모든 연속 K일 구간을 다 구해보면 되지 않을까?<br><br>i=1일 때: arr[1]+arr[2]+...+arr[K]<br>i=2일 때: arr[2]+arr[3]+...+arr[K+1]<br>...<br>각 구간마다 K개를 더하면 합을 구할 수 있어!' },
                { title: 'But there\'s a problem with this', content: '구간이 N-K+1개이고, 각 구간마다 K개를 더하니까 전체 <strong>O(N × K)</strong>.<br>N이 100,000이고 K도 비슷하면... 꽤 느릴 수 있어!<br><br>그리고 매번 K개를 처음부터 다 더하는 건 낭비야 — 이전 구간이랑 겹치는 부분이 K-1개나 되는데 또 더하고 있으니까.' },
                { title: 'What if we try this?', content: '누적합을 미리 만들어두면 길이 K인 구간 합을 <strong>뺄셈 한 번</strong>으로 구할 수 있어!<br><br>i번째부터 K개 합 = <code>prefix[i + K - 1] - prefix[i - 1]</code><br><br>이걸 i = 1부터 N-K+1까지 반복하며 최대값을 갱신하면 끝!<br>전처리 O(N) + 탐색 O(N) = <strong>O(N)</strong> ⚡<br><br>💡 참고: 슬라이딩 윈도우(앞 하나 빼고 뒤 하나 더하기)로도 O(N)에 풀 수 있지만, 누적합을 쓰면 코드가 더 간결해!' }
            ],
            templates: {
        ...
        // ========== 2단계: 응용 ==========
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '질문마다 l부터 r까지 훑으면서 해당 알파벳이 나올 때마다 카운트하면 되지 않을까?<br><br><strong>예: "seungjaehwang"에서 \'a\', 0~12</strong><br>→ s(X) e(X) u(X) n(X) g(X) j(X) <strong>a(O)</strong> e(X) h(X) w(X) <strong>a(O)</strong> n(X) g(X) → 2개<br>한 번의 질문에 구간 길이만큼 확인하면 돼!' },
                { title: 'But there\'s a problem with this', content: '문자열 길이 최대 200,000, 질문도 최대 200,000번이야.<br>매번 구간을 훑으면 200,000 × 200,000 = <strong>400억 번</strong>... 시간 초과!<br><br>앞 문제에서 배운 누적합을 쓰면 될 것 같은데, 여기는 합이 아니라 <strong>"특정 문자의 개수"</strong>잖아. 어떻게 누적합을 적용하지?' },
                { title: 'What if we try this?', content: '숫자의 합 대신 <strong>문자의 등장 횟수</strong>를 누적하면 돼!<br><br>알파벳이 26개니까, <strong>26개의 누적합 배열</strong>을 만들자:<br><code>count[c][i]</code> = 처음부터 i번째까지 알파벳 c가 나온 횟수<br><br>그러면 l~r 구간에서 알파벳 c의 개수 = <code>count[c][r+1] - count[c][l]</code><br>— 뺄셈 한 번이면 O(1)! 전처리 O(26 × N) + 쿼리 O(Q) = 충분히 빨라!<br><br>💡 인덱스가 0부터 시작하니까, 누적합 배열 길이를 N+1로 만들면 편해.' }
            ],
            templates: {
        ...
        },
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '모든 (i, j) 쌍을 시도해서 구간 합을 구한 뒤, M으로 나누어 떨어지는지 확인하면 되지 않을까?<br><br>이중 for문으로 i와 j를 정하고, 누적합으로 구간 합 = <code>prefix[j] - prefix[i]</code>를 구해서 M으로 나눠보자.' },
                { title: 'But there\'s a problem with this', content: 'N이 최대 <strong>1,000,000</strong>(백만)이야!<br>모든 (i, j) 쌍은 약 N² / 2 = <strong>5000억 개</strong>... 절대 불가능!<br><br>그런데 잠깐, 구간 합이 M의 배수라는 건 <code>prefix[j] - prefix[i]</code>가 M의 배수라는 거잖아?<br>이걸 다르게 표현하면... <code>prefix[j] % M == prefix[i] % M</code>이 되네!' },
                { title: 'What if we try this?', content: '누적합의 <strong>나머지가 같은 것끼리 짝</strong>을 지으면 돼!<br><br>① 각 prefix 값을 M으로 나눈 나머지를 구해<br>② <code>cnt[r]</code> = 나머지가 r인 prefix 값의 개수를 세어<br>③ 나머지가 같은 것 중 2개를 고르면 구간 합이 M의 배수!<br>&nbsp;&nbsp;&nbsp;→ 답 = 각 나머지별 <code>cnt[r] × (cnt[r]-1) / 2</code>를 모두 합산<br><br>⚠️ <code>prefix[0] = 0</code>도 빼먹으면 안 돼! 나머지 0에 포함시켜야 해.<br><span class="lang-cpp">답이 매우 커질 수 있으니 <code>long long</code> 타입 필수!</span><span class="lang-py">Python은 큰 수를 자동 처리하니 걱정 없어!</span>' }
            ],
            templates: {
        ...
        // ========== 3단계: 2차원 ==========
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '(x1,y1)부터 (x2,y2)까지의 합을 구하라고 하니까, 이중 for문으로 직사각형 영역을 다 더하면 되지 않을까?<br><br><strong>예: (2,2)~(3,4) 영역</strong><br>for i in range(x1, x2+1):<br>&nbsp;&nbsp;for j in range(y1, y2+1):<br>&nbsp;&nbsp;&nbsp;&nbsp;합 += arr[i][j]<br>한 번의 쿼리에 영역 크기만큼 더하면 돼!' },
                { title: 'But there\'s a problem with this', content: 'N이 1,024이니까 영역 크기가 최대 1,024 × 1,024 = 약 <strong>100만</strong>이야.<br>쿼리가 100,000번이면 100만 × 10만 = <strong>1000억 번 연산</strong>... 시간 초과!<br><br>1차원에서는 누적합으로 구간 합을 O(1)에 구했잖아. 2차원에서도 비슷하게 할 수 있을까?' },
                { title: 'What if we try this?', content: '<strong>2차원 누적합</strong>을 만들면 돼! 개념 페이지에서 배운 <strong>포함-배제</strong> 원리를 그대로 적용하자.<br><br><strong>① 누적합 만들기:</strong><br><code>prefix[i][j] = arr[i][j] + prefix[i-1][j] + prefix[i][j-1] - prefix[i-1][j-1]</code><br>(위 + 왼쪽 - 겹치는 부분 + 현재값)<br><br><strong>② 쿼리 답하기:</strong><br><code>prefix[x2][y2] - prefix[x1-1][y2] - prefix[x2][y1-1] + prefix[x1-1][y1-1]</code><br>(전체 - 위쪽 - 왼쪽 + 다시 더해진 모서리)<br><br>전처리 O(N²) + 쿼리 O(1) × M = <strong>O(N² + M)</strong>!' }
            ],
            templates: {
        ...
        },
        {
            ...
            `,
            hints: [
                { title: 'First intuition', content: '모든 K×K 영역을 하나씩 잡고, 그 안에서 체스판 패턴과 다른 칸이 몇 개인지 세면 되지 않을까?<br><br>체스판 패턴은 2가지(왼쪽 위가 B / 왼쪽 위가 W)니까, 각 패턴에 대해 다른 칸 수를 구하고 더 작은 쪽을 선택하면 돼.' },
                { title: 'But there\'s a problem with this', content: 'K×K 영역의 수 = (N-K+1) × (M-K+1)개, 각 영역마다 K×K칸을 확인...<br>N, M이 최대 2,000이면 전체 <strong>O(N × M × K²)</strong> — 너무 느려!<br><br>그런데 잠깐, "K×K 영역의 합"... 이거 앞 문제(구간 합 구하기 5)에서 한 것 아닌가? 2차원 누적합으로 O(1)에 구할 수 있었잖아!' },
                { title: 'What if we try this?', content: '① 먼저 체스판 패턴과 <strong>다른 칸을 1, 같은 칸을 0</strong>으로 표시한 배열을 만들자.<br>&nbsp;&nbsp;&nbsp;(i+j)가 짝수면 B여야 하고, 홀수면 W여야 하는 패턴 기준!<br><br>② 이 0/1 배열의 <strong>2차원 누적합</strong>을 구해!<br><br>③ 모든 K×K 영역에서 cost1 = 누적합으로 O(1)에 구한 "다른 칸 수"<br>&nbsp;&nbsp;&nbsp;반대 패턴의 비용은 <code>K×K - cost1</code>이니까 누적합을 한 번만 만들면 돼!<br>&nbsp;&nbsp;&nbsp;→ <code>min(cost1, K×K - cost1)</code> 중 최소값이 답!<br><br>전처리 O(N×M) + 탐색 O(N×M) = <strong>O(N×M)</strong>!' }
            ],
            templates: {
